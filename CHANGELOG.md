...
- Optional retention: `tokenWatch.retention.rawRecordDays` drops per-turn detail
  older than the days you set. Daily and per-session totals are always kept, so
  your history stays complete — a pruned day just loses its hourly and tool-call
  drill-down. Off by default.
- Codex limit resets that are about to expire are announced once, so a granted
  reset is not quietly lost.
- The Codex and Claude Code cards show your account plan next to the title, for
  example `Codex Usage (Pro Lite)`, and so does the status bar tooltip. It comes
  from your local sign-in, so it shows even when usage data cannot be loaded.
- A new Token Watch logo, used for the Marketplace listing, the activity bar,
  and the status bar; Codex and Claude Code have their own icons too.
- Bundled pricing for Claude Fable 5.1, Claude Mythos 5.1 and Codex Auto
  Review, and updated pricing for GPT-5.6 Terra and GPT-5.6 Luna.

### Changed

- Existing databases shrink by around 40% the first time this version opens
  them, with no setting to change. With retention on top, more.
- **Reload the window after upgrading.** The database format changed, and an
  older window left running would double-count turns it re-reads.
- Limit resets moved behind "show more" on the usage card. The card keeps the
  count and the soonest expiry, which is the part that is a deadline.
- Panel and chart colours now follow your editor theme.
- Top models no longer offers "View all models" when every model is listed.
- "Reset Database" leaves an empty file instead of one as large as the database
  it replaced.

### Fixed

- **Codex token counts were about 39% too high** when sessions were resumed.
  Totals correct themselves on the next scan — nothing to rescan by hand.
- Turns written on very long log lines were skipped entirely and are now
  counted, so totals no longer miss tokens from large file writes.
- The dashboard opens on real numbers instead of a spinner, and no longer times
  out during a first scan of a large log set.
- Switching period or source is instant: results already fetched are reused
  rather than queried again, and stale figures stay on screen while they
  refresh.
- Quota figures keep refreshing. They used to load once and then freeze until
  you pressed refresh or reopened the panel.
- Each provider is polled at its own pace, and reopening the sidebar no longer
  fires extra calls — which is what got Claude rate-limited. A refused request
  now backs off gently instead of waiting the full 15 minutes.
- Warnings that pointed at data that was never missing are gone — skipped long
  lines, unpriced models, and lines from a session still being written. What is
  genuinely missing is reported in "Token Watch: Show Diagnostics".
- Interrupted writes no longer leave copies of the database behind; one install
  had accumulated 1.5 GB of them.
